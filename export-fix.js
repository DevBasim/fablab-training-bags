  ...
  const exportWorkbook=()=>{
    try{
      ...
      if(Array.isArray(appState.readiness)&&appState.readiness.length){
        rows.push([],['متطلبات الجاهزية قبل التنفيذ']);
        appState.readiness.forEach((r,i)=>{
          const category=clean(r?.category)||`متطلب الجاهزية ${i+1}`;
          rows.push([category]);
          if(r?.preparation_requirements)rows.push(['ما يجب تجهيزه أو التحقق منه',clean(r.preparation_requirements)]);
          if(r?.lead_time)rows.push(['المدة اللازمة قبل التنفيذ',clean(r.lead_time)]);
          if(r?.notes)rows.push(['ملاحظات',clean(r.notes)]);
          if(r?.is_ready===true)rows.push(['الجاهزية','نعم']);
          rows.push([]);
        });
      }
