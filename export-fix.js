...
(state.outputs||[]).forEach((o,i)=>{const r=29+i;[i+1,o.name,o.output_type,o.description,o.quantity,o.ownership,`ما الذي سنقيسه؟ ${o.measurement?.what_to_measure||''}\nالنتيجة المطلوبة: ${o.measurement?.required_result||''}\nطريقة التحقق: ${o.measurement?.verification_method||''}`].forEach((x,j)=>put(r,j+1,x))});
section(42,'قياس المخرجات');['رقم المخرج / الاسم','ما الذي سنقيسه؟','النتيجة المطلوبة','طريقة التحقق'].forEach((x,i)=>put(43,i+1,x));
(state.outputs||[]).forEach((o,i)=>{const r=44+i;[`${i+1} - ${o.name||''}`,o.measurement?.what_to_measure||'',o.measurement?.required_result||'',o.measurement?.verification_method||''].forEach((x,j)=>put(r,j+1,x))});
...
(state.days||[]).forEach((d,i)=>{const r=53+i;[d.day_no,d.what_to_learn,d.topics,(d.objective_ids||[]).map(x=>Number(x)+1).join(', '),(d.output_ids||[]).join(', '),d.execution_duration,d.verification].forEach((x,j)=>put(r,j+1,x))});
section(65,'الموارد البشرية');['المورد البشري','العدد','الخبرة أو الشروط المطلوبة','المهام'].forEach((x,i)=>put(66,i+1,x));
(state.human||[]).forEach((h,i)=>[h.resource_type,h.quantity,h.requirements,h.responsibilities].forEach((x,j)=>put(67+i,j+1,x)));
...
(state.material||[]).forEach((m,i)=>[m.item,m.specifications,m.unit,m.quantity_per_person_group,m.total_quantity,m.notes].forEach((x,j)=>put(72+i,j+1,x)));
...
(state.readiness||[]).forEach((r,i)=>[r.category,r.is_ready?'نعم':'لا',r.preparation_requirements,r.lead_time,r.notes].forEach((x,j)=>put(81+i,j+1,x)));
section(90,'المرفقات');['نوع المرفق','اسم الملف','ملاحظة'].forEach((x,i)=>put(91,i+1,x));
Object.keys(state.attachments||{}).forEach((k,i)=>{const f=state.attachments[k];if(f)[k,f.name,'مرفق مع الحقيبة'].forEach((x,j)=>put(92+i,j+1,x))});
section(100,'المراجعة والاعتماد');put(101,1,'الحالة');put(101,2,'ملاحظات المراجعة');put(102,1,$('reviewStatus')?.selectedOptions?.[0]?.text||'');put(102,2,T('reviewComment'));
section(105,'قائمة التحقق');[['☐','التأكد من اكتمال بيانات الحقيبة'],['☐','مراجعة الأهداف والمخرجات وخطة التنفيذ'],['☐','التأكد من الجاهزية والمرفقات']].forEach((x,i)=>{put(106+i,1,x[0]);put(106+i,2,x[1])});
...
function bind(){const b=$('exportExcel');if(b)b.onclick=exportWorkbook}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bind);else bind();})();